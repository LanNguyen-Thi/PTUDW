...
module.exports = {
    ...
    const rows = await db.load(`select *, DATE_FORMAT(k.NgayCapNhat, "%M %d %Y") as LastUpdate, round(k.HocPhi*(1-k.KhuyenMai)) as giaMoi, DATE_FORMAT(g.NgaySinh, "%M %d %Y") as NgSinh
    from khoahoc k, giangvien g, linhvuccap2 l
    where k.NguoiDay = g.idGiangVien and k.IdKhoaHoc =  ${idKhoaHoc} and k.HienThi=1 and l.idLinhVucCap2 = k.LinhVuc`);
    if (rows.length === 0)
        return null;
    return rows[0];
    },
    ...
    async relatedCourse(idKhoaHoc) {
        const rows = await db.load(`SELECT *, round(k.HocPhi*(1-k.KhuyenMai),2) as giaMoi, DATE_FORMAT(k.NgayCapNhat, "%M %d %Y") as LastUpdate
    ...
    async listOfChapter(idKhoaHoc) {
        const rows = await db.load(`select c.idChuongKhoaHoc, c.TenChuong, k.IdKhoaHoc
    ...
    async checkComment(idHocVien,IdKhoaHoc) {
        const rows = await db.load(`select * from hocvien_danhgia_khoahoc where HocVien_idHocVien = ${idHocVien} and KhoaHoc_IdKhoaHoc =${IdKhoaHoc}`);
        if (rows.length === 0)
            return true;
        return false;
        },
